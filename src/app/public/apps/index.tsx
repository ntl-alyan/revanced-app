...
import { ChevronRight, Search, Filter, Smartphone, Package2, Download, Star, ArrowRight, ExternalLink, Github } from "lucide-react";
...
export default function PublicAppsPage() {
  ...
  const appsTitle = settings?.find((s: any) => s.settingKey === "apps_page_title")?.settingValue || "ReVanced Apps";
  const appsDescription = settings?.find((s: any) => s.settingKey === "apps_page_description")?.settingValue || 
    "Discover our collection of enhanced applications offering improved functionality and features.";

  // Filter apps based on search query
  let filteredApps = apps?.filter((app: App) => 
    app.name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    (app.description && app.description.toLowerCase().includes(searchQuery.toLowerCase()))
  ) || [];

  // Sort apps based on selected option
  filteredApps = filteredApps.sort((a: App, b: App) => {
    if (sortBy === "name") {
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="rounded-xl overflow-hidden border border-white/10 bg-[#111827] shadow-lg">
                  ...
                  <div className="rounded-xl overflow-hidden border border-white/10 bg-gradient-to-br from-[#111827] to-[#0f172a] shadow-lg hover:shadow-xl transition-all duration-300 hover:border-primary/30 hover:-translate-y-1 h-full flex flex-col relative group">
                    ...
                    <div className="p-5 flex items-center justify-between relative z-10">
                      ...
                      <div className="h-14 w-14 rounded-xl bg-black/40 backdrop-blur-sm border border-white/10 flex items-center justify-center overflow-hidden shadow-lg">
                        {app.icon ? (
                          <img src={app.icon} alt={`${app.name} icon`} className="h-8 w-8" />
                        ) : (
                      ...
                      <div className="flex flex-col items-end space-y-1">
                        {app.version && (
                          <Badge variant="outline" className="bg-black/20 text-white/90 border-white/10 backdrop-blur-sm">
                            v{app.version}
                          </Badge>
                        )}
                    ...
                    <div className="p-5 pt-2 flex-grow relative z-10">
                      <h3 className="text-xl font-semibold text-white mb-3 group-hover:text-primary transition-colors">{app.name}</h3>
                      <div className="flex flex-wrap gap-2 mb-4">
                        ...
                        {Array.isArray(app.sections) && app.sections.some((s: AppSection) => s?.type === 'features') && (
                          <Badge variant="secondary" className="bg-primary/20 text-primary border-none">
                            <Star className="mr-1 h-3 w-3" /> Premium Features
                          </Badge>
                        )}
                        
                        {Array.isArray(app.sections) && app.sections.some((s: AppSection) => s?.type === 'installation') && (
                    ...
                    <div className="p-5 pt-0 relative z-10">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-white/50">
                          {app.version ? `Latest: v${app.version}` : "Ad-free experience"}
                        </span>
                      ...
                      <div className="mt-4 h-1 w-full bg-gray-800 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-primary/70 to-primary rounded-full" style={{ width: '70%' }}></div>
                      </div>
            ...
            <div className="text-center py-12 max-w-5xl mx-auto">
              <div className="bg-[#111827] border border-white/10 p-10 rounded-xl relative overflow-hidden">
                ...
                <h3 className="text-xl font-medium mb-2 text-white">No apps found</h3>
                <p className="text-white/70 mb-6 max-w-md mx-auto">
                  No apps match your search criteria. Try a different search term or check back later for new additions.
                </p>
                {searchQuery && (
                  <Button onClick={() => setSearchQuery("")} variant="outline" className="bg-transparent border-white/20 text-white hover:bg-white/10">
                    Clear Search
                  </Button>
                )}
              </div>
