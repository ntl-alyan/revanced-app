...
export default function CategoryPostsPage() {
  ...
  return (
    <PublicLayout>
      <Helmet>
        <title>{category ? `${category.name} - Blog` : 'Category - Blog'} - ReVanced</title>
        <meta 
          name="description" 
          content={category ? `Browse all ${category.name} posts on the ReVanced blog` : 'Browse posts by category on the ReVanced blog'} 
        />
      </Helmet>
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-br from-white to-white/70">
            {isLoadingCategory ? (
              ...
              category ? `${category.name} Articles` : 'Category'
            )}
          </h1>
      ...
      <div className="container mx-auto px-4 py-16">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-white/70 mb-8">There are currently no posts in the {category?.name} category.</p>
