...
export default function DownloadPage() {
  ...
  const getSetting = (key: string): string | undefined => {
    const setting = settings?.find((s: { settingKey: string; }) => s.settingKey === key);
  ...
  const getAppIcon = (appData: any) => {
    if (appData?.icon) {
      return <img src={appData.icon} alt={`${appData.name} icon`} className="h-20 w-20" />;
  ...
  const pageTitle = getSetting('downloads_page_title') || `Download ${app.name}`;
  
  return (
    <PublicLayout>
      <Helmet>
        <title>{pageTitle} | ReVanced</title>
        <meta name="description" content={app.metaDescription || `Download ${app.name} - ${app.description?.substring(0, 160)}`} />
        <meta name="robots" content="noindex, nofollow" />
        {/* Open Graph meta tags */}
        <meta property="og:type" content="website" />
        <meta property="og:title" content={`${pageTitle} | ReVanced`} />
        <meta property="og:description" content={app.metaDescription || `Download ${app.name} - ${app.description?.substring(0, 160)}`} />
              ...
              <div className="bg-black/20 border border-primary/10 rounded-2xl p-6 md:p-8 backdrop-blur-sm mb-6">
                ...
                {preparingDownload ? (
                  <div className="space-y-4">
                    ...
                    <Progress value={downloadProgress} className="h-3 bg-black/50" />
                    <p className="text-sm text-white/60 text-center mt-3">
                      Your download will be ready in {Math.ceil(10 - (downloadProgress / 10))} seconds
                    </p>
                  </div>
                ) : app.downloadUrl ? (
                  ...
                  <AccordionContent className="px-6 pb-6">
                    <div className="mt-2 space-y-5">
                      ...
                      {app.name?.includes("MicroG") ? (
                        <>
                          <div className="text-lg font-medium text-primary mb-3">MicroG Installation</div>
                          <ol className="list-decimal pl-5 space-y-3 text-white/80">
                            ...
                            <li>Once installed, you can proceed to install any ReVanced apps</li>
                          </ol>
                          <div className="mt-4 p-4 bg-primary/10 border border-primary/20 rounded-xl text-white/70">
                            <p>MicroG is required for ReVanced apps to function properly as it provides the necessary Google services without requiring the official Google Play Services.</p>
                          </div>
                        </>
                      ) : app.name?.includes("ReVanced") ? (
                        <>
                          <div className="text-lg font-medium text-primary mb-3">ReVanced App Installation</div>
                          <ol className="list-decimal pl-5 space-y-3 text-white/80">
                            <li>Ensure you have installed <strong>ReVanced MicroG</strong> first (required for proper functioning)</li>
                            <li>Download the ReVanced APK to your Android device</li>
                            <li>Go to <strong>Settings → Security</strong> and enable <strong>"Install from unknown sources"</strong> for your browser</li>
                            <li>Open the downloaded APK file and follow the prompts to install</li>
                            <li>If you receive a "Replace app" prompt, click "OK" to update the existing installation</li>
                          ...
                          <div className="mt-4 p-4 bg-yellow-500/10 border border-yellow-500/20 rounded-xl text-white/80">
                            ...
                            <p>Important: Make sure to uninstall any official YouTube app before installing ReVanced to avoid conflicts.</p>
                          </div>
                        </>
                      ) : app.name?.includes("Manager") ? (
                        <>
                          <div className="text-lg font-medium text-primary mb-3">ReVanced Manager Installation</div>
                          <ol className="list-decimal pl-5 space-y-3 text-white/80">
                            <li>Download ReVanced Manager to your Android device</li>
                            <li>Go to <strong>Settings → Security</strong> and enable <strong>"Install from unknown sources"</strong> for your browser</li>
                            <li>Open the downloaded APK file and follow the prompts to install</li>
                            <li>Launch ReVanced Manager after installation</li>
                            <li>Follow the in-app instructions to patch and install your desired apps</li>
                          </ol>
                          <div className="mt-4 p-4 bg-primary/10 border border-primary/20 rounded-xl text-white/70">
                            <p>ReVanced Manager allows you to create your own customized versions of apps with only the patches you want.</p>
                          </div>
                        </>
                      ) : (
                        <>
                          <div className="text-lg font-medium text-primary mb-3">Android App Installation</div>
                          <ol className="list-decimal pl-5 space-y-3 text-white/80">
                            <li>Download the app to your Android device</li>
                            <li>Go to <strong>Settings → Security</strong> and enable <strong>"Install from unknown sources"</strong> for your browser</li>
                            <li>Open the downloaded APK file and follow the prompts to install</li>
                            <li>Once installed, you can find and launch the app from your app drawer</li>
                          </ol>
                        </>
                      )}
                      ...
                      <div className="mt-6 pt-5 border-t border-primary/10">
                        <h4 className="font-medium mb-3 text-white/90">Troubleshooting</h4>
                        <p className="mb-3 text-white/80">If you encounter any installation issues:</p>
                        <ul className="list-disc pl-5 space-y-2 text-white/70">
                          <li>Make sure your device has enough storage space</li>
                          <li>Check if you have enabled installation from unknown sources</li>
                          <li>Verify that you're using a supported Android version</li>
                          <li>Try downloading the file again if it appears corrupted</li>
                        </ul>
                      </div>
                    </div>
                  </AccordionContent>
